...
var $author$project$Main$numberToID = A2(
...
var $author$project$Main$pipeUrgency = A2(
	$elm$json$Json$Decode$andThen,
	function (urgencyString) {
		...
		$elm$http$Http$get(
			{
				...
				url: 'http://192.168.0.222:5000/active'
			}));
};
...
var $author$project$Main$NewNumber = F2(
	function (a, b) {
		return {$: 'NewNumber', a: a, b: b};
	});
...
var $author$project$Main$randomNumber = A2($elm$random$Random$int, 1, 9999);
var $author$project$Main$newId = function (buffer) {
	return A2(
		$elm$random$Random$generate,
		$author$project$Main$NewNumber(buffer),
...
var $author$project$Main$encodeId = function (id) {
	return $elm$json$Json$Encode$int(id.id);
};
var $elm$json$Json$Encode$string = _Json_wrap;
var $author$project$Main$encodeUrgency = function (urgency) {
	switch (urgency.$) {
		case 'Low':
		...
		case 'High':
			return $elm$json$Json$Encode$string('high');
		default:
			return $elm$json$Json$Encode$string('closed');
	}
};
...
var $author$project$Main$statusEncoder = function (quote) {
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				_Utils_Tuple2(
				'id',
				$author$project$Main$encodeId(quote.id)),
				_Utils_Tuple2(
				'urgency',
				$author$project$Main$encodeUrgency(quote.urgency))
			]));
};
var $author$project$Main$postCloseRequest = function (quote) {
	return $elm$http$Http$post(
		{
			body: $elm$http$Http$jsonBody(
				$author$project$Main$statusEncoder(quote)),
			expect: $elm$http$Http$expectWhatever($author$project$Main$Sent),
			url: 'http://192.168.0.222:5000/close'
		});
};
	...
	return $elm$json$Json$Encode$object(
		_List_fromArray(
			[
				...
				_Utils_Tuple2(
				'quote',
				$elm$json$Json$Encode$string(quote.quote)),
				_Utils_Tuple2(
				'source',
				$elm$json$Json$Encode$string(quote.source)),
				_Utils_Tuple2(
				'author',
				$elm$json$Json$Encode$string(quote.author)),
				_Utils_Tuple2(
				'year',
				$elm$json$Json$Encode$int(quote.year)),
				_Utils_Tuple2(
				'updateList',
	...
	return $elm$http$Http$post(
		{
			...
			expect: $elm$http$Http$expectWhatever($author$project$Main$Sent),
			url: 'http://192.168.0.222:5000/new'
		});
};
	...
	return $elm$http$Http$post(
		{
			body: $elm$http$Http$jsonBody(
				$author$project$Main$statusEncoder(quote)),
			expect: $elm$http$Http$expectWhatever($author$project$Main$Sent),
			url: 'http://192.168.0.222:5000/status'
		});
};
...
var $author$project$Main$postUpdate = function (quote) {
	return $elm$http$Http$post(
		{
			body: $elm$http$Http$jsonBody(
				$author$project$Main$updateEncoder(quote)),
			expect: $elm$http$Http$expectWhatever($author$project$Main$Sent),
			url: 'http://192.168.0.222:5000/update'
		});
};
var $author$project$Main$requestFactory = F3(
	function (buffer, name, id) {
	...
	function (msg, model) {
		switch (msg.$) {
			case 'CreateRequest':
				...
				return _Utils_Tuple2(
					model,
					$author$project$Main$newId(currentBuffer));
			case 'NewNumber':
				var buff = msg.a;
				var id = msg.b;
				return _Utils_Tuple2(
								...
								$elm$core$List$concat(
									_List_fromArray(
										[
											_List_fromArray(
											[
												A3($author$project$Main$requestFactory, buff, model.name, id)
											]),
											function () {
			...
			case 'ToggleDead':
				return (model.deadbool === 1) ? _Utils_Tuple2(
					...
					$elm$http$Http$get(
						{
							expect: A2($elm$http$Http$expectJson, $author$project$Main$DataReceived, $author$project$Main$decodeListQuote),
							url: 'http://192.168.0.222:5000/inactive'
						})) : _Utils_Tuple2(
					_Utils_update(
						model,
						{deadbool: 1}),
